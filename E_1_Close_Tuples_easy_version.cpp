...
void ps(vector<int>& v) {
    ...
    // all 3 elements are same

    // for (auto& itr : h) {

    //     int k1 = itr.second;
    //     ll temp = k1 * (k1 - 1) * (k1 - 2) / 6;
    //     ans += temp;
    ...
    for (int i = 1;i <= n;i++) {
        unsigned ll k1 = h[i];
        unsigned ll temp = (k1 * (k1 - 1) * (k1 - 2)) / 6;
        ans += temp;
    ...
    //     // int k1 = itr.second;
    //     // ll temp = k1 * (k1 - 1) / 2;
    //     // temp *= (h[itr.first - 1] + h[itr.first - 2] + h[itr.first + 2] + h[itr.first + 1]);
    ...
    for(int i=1;i<=n;i++){
         unsigned ll k1=h[i];
       unsigned ll temp = k1 * (k1 - 1) / 2;
        temp*= ((i-1)>=0 ? h[i-1] : 0)+((i-2)>=0 ? h[i-2] : 0) + ((i+1)<=n ? h[i+1] : 0) + ((i+2)<=n ? h[i+2] : 0) ;
    ...
    //     // ll temp= itr.second * h[itr.first + 1] *h[itr.first+2];
