...
int ps(string& s, string& t) {
    vector<int>v(t.size(), -1);
    map<char,vector<int>>char_pos;
    ...
    v[v.size()-1]=char_pos[t[t.length()-1]].back();
    ans=max(ans,v[v.size()-1]-v[v.size()-2]);
    int n=v.size();
    for(int i=n-2;i>=1;i--){
        vector<int>& pos_vec=char_pos[t[i]];
        int index=lower_bound(pos_vec.begin(),pos_vec.end(),v[i+1])-pos_vec.begin();
        index--;
        v[i]=pos_vec[index];
