...
void ps(int l, int r, map<int, int>& left_socks, map<int, int>& right_socks) {
    int ans=0;

    for (auto& itr : left_socks) {

        int x = min(itr.second, right_socks[itr.first]);

        itr.second -= x;
        right_socks[itr.first] -= x;
        l -= x;
        r -= x;
        // ans+=x;
    ...
    for (auto& itr : right_socks) {
        int x = min(itr.second, left_socks[itr.first]);

        itr.second -= x;
        left_socks[itr.first] -= x;
        l -= x;
        r -= x;
        // ans+= x;
...
int main() {
    ...
    while (t--) {
        ...
        map<int, int>left_socks, right_socks;
        for (int i = 0;i < l;i++) {
            int x;
            cin >> x;
            left_socks[x]++;
